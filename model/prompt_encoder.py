...
class Prototype_Prompt_Encoder(nn.Module):
    ...
    def forward(self, feat, prototypes, cls_ids):
        ...
        #print("Feat shape:", feat.shape)  # 打印feat的形状
...
class Prototype_Prompt_Encoder(nn.Module):
    def __init__(self, feat_dim=256, num_tokens=8):
        super(Prototype_Prompt_Encoder, self).__init__()
        
        # Initialize embedding layer for positive classes
        self.cls_embeddings = nn.Embedding(num_tokens, feat_dim)

    def forward(self, cls_ids):
        # Use cls_ids to get embeddings from the class embeddings
        cls_embeddings = self.cls_embeddings(cls_ids)
        
        # Return the embeddings corresponding to the class IDs

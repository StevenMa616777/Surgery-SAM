...
def create_endovis_masks(binary_masks, H, W):
    ...
    Returns: a dictionary containing one prediction mask for each frame with the frame name as key and its predicted mask as value; 
             ...
             The prediction mask for each frame is a 1024 x 1024 map with each value representing the class id for the pixel;
    ...
    for seq in binary_masks.keys():
        ...
        for frame in binary_masks[seq].keys():
            ...
            for binary_mask in binary_masks_list:
                ...
                predicted_label = int(re.search(r"class_(\d+)", mask_name).group(1))
                mask = binary_mask["mask"].numpy()

                mask_resized = cv2.resize(mask, (W, H), interpolation=cv2.INTER_NEAREST)

                # After resizing, the mask might not be binary (0 and 1) due to interpolation
                # So, we threshold it to make sure it's binary
                mask_resized = (mask_resized > 0.5).astype(np.uint8)

                # Now we can safely use mask to index into endovis_mask
                endovis_mask[mask_resized == 1] = predicted_label
